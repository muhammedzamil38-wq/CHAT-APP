          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-primary/20 text-primary uppercase tracking-wider">Admin</span>
              </h2>
              <p className="text-xs text-muted-foreground">Manage users, moderation reports, and system analytics.</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
              ...
              <p className="text-sm font-medium tracking-widest uppercase">Loading System Intelligence...</p>
            </div>
          ) : activeTab === 'overview' ? (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="p-6 rounded-2xl bg-primary/5 border border-primary/10 relative overflow-hidden group">
                  ...
                  <p className="text-[10px] font-bold text-primary uppercase tracking-widest mb-1">Active Users</p>
                  <h3 className="text-3xl font-bold text-foreground flex items-baseline gap-2">
                    {onlineUsers.length}
                    <span className="text-[10px] text-emerald-500 animate-pulse font-mono uppercase">Online</span>
                  </h3>
                </div>
                ...
                <div className="p-6 rounded-2xl bg-card border border-border/40">
                  <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest mb-1">Pending Reports</p>
                  <h3 className="text-3xl font-bold text-foreground">{analytics?.stats.totalReports || 0}</h3>
                </div>
                <div className="p-6 rounded-2xl bg-card border border-border/40">
                  <p className="text-[10px] font-bold text-amber-500 uppercase tracking-widest mb-1">Banned Accounts</p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="p-6 rounded-2xl bg-card border border-border/40 flex flex-col h-[400px]">
                  <div className="flex items-center justify-between mb-6">
                    <h4 className="text-sm font-bold uppercase tracking-wider flex items-center gap-2">
                      <Activity className="w-4 h-4 text-primary" /> System Traffic (Messages)
                    </h4>
                  </div>
                ...
                <div className="p-6 rounded-2xl bg-card border border-border/40 flex flex-col h-[400px]">
                  <div className="flex items-center justify-between mb-6">
                    <h4 className="text-sm font-bold uppercase tracking-wider flex items-center gap-2">
                      <Users className="w-4 h-4 text-amber-500" /> User Growth (New Accounts)
                    </h4>
                  </div>
                  <div className="flex-1">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={formatChartData(analytics?.trends.users)}>
                        ...
                        <Tooltip 
                          ...
                          itemStyle={{ fontSize: '10px', fontWeight: 'bold', textTransform: 'uppercase' }}
                        />
                        <Line type="monotone" dataKey="count" name="New Users" stroke="#f59e0b" strokeWidth={2} dot={{ r: 4, fill: '#f59e0b' }} activeDot={{ r: 6 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  toast.success(`User status updated: ${!user.isBanned ? 'BANNED' : 'UNBANNED'}`);
                                } catch (error) {
                        ...
                        <Button
                          ...
                          onClick={async () => {
                             try {
                               ...
                               toast.success(`User status updated: ${!report.isBanned ? 'BANNED' : 'UNBANNED'}`);
                             } catch (error) {
                               console.error('Failed to update ban status', error);
                         ...
                         >
                           {report.isBanned ? 'Unban User' : 'Ban User'}
                         </Button>
                      </td>
                    </tr>
                  ))}
